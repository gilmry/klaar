...
- **0.5** — harnais contrat API : `klaar-api` sert désormais un vrai endpoint (`/api/v1/health`), sa doc utoipa (`/api/v1/openapi.json`) et Swagger UI (`/api/v1/docs/`), fuzzé par `schemathesis` en CI. **Limites connues** :
  ...
  - `cargo audit` ignore aussi **RUSTSEC-2023-0071** (rsa 0.9.10, attaque Marvin), et pour une raison plus forte que la précédente : **ce code n'est jamais construit**. `rsa` n'est atteignable que par `sqlx-mysql`, pilote qu'une application PostgreSQL n'active pas. `cargo audit` lit `Cargo.lock`, qui enregistre les dépendances optionnelles qu'elles soient activées ou non : retirer la fonctionnalité `macros` de sqlx — essayée pour cela — ne l'en sort pas. Deux commandes le vérifient plutôt que de le croire : `cargo tree -i rsa --workspace --target all` ne rend rien, et `target/debug/deps` ne contient ni `librsa-` ni `libsqlx_mysql-` (seulement `libsqlx_postgres`). À revoir si un pilote MySQL est un jour activé, auquel cas l'avis redeviendrait réel
